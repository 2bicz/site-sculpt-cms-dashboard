...
export const Pages = () => {
    ...
    useEffect(() => {
      if (websiteId) {
        api.get(`/page/get-all-by-website/${websiteId}`)
        .then((response) => {
    ...
    const prepareTableRow = ({ order, title, path, pageId }) => {
      return {
        order: (<SoftTypography variant="caption" color="secondary" fontWeight="medium">{ order }</SoftTypography>),
        title: (<SoftTypography variant="caption" color="secondary" fontWeight="medium">{ title }</SoftTypography>),
        path: (<SoftTypography variant="caption" color="secondary" fontWeight="medium">{ path }</SoftTypography>),
        "edit page info": (
            <SoftButton 
              onClick={() => {
